...
gui,Add,Text,+E0x00000020 +hwndtxthw w%wtemp% h%A_H_HC% vMH
,% SizingTemplate
guiControlGet,MH, Pos ; -	-get dummy size. -	-	-	(Will have to come out probably)
guiControl,,MH ;WinSet,Transcolor,000000;winset,ExStyle,-0x80000,ahk_id %Gui_hW%
...
gshow(hw="",xx="",yy="",ww="",hh="") {
	global DTOP,gui_xx,gui_yy,gui_hh,gui_hw,txthwnd,SizingTemplate
	...
	else,winset,style,-0x80000000,ahk_id %hw%
	sleep,35
}
...
Window2Dtop(byref Child="",x:=1,y:=45,w:=480,h:=480){		; Window2Dtop(byref Child="",x:=1,y:=45,w:=480,h:=480){	
	...
	Win_Move(Child,x,y,"","","")				; msgbox %  Child " " ChildX " " ChildY " " Child_W " " Child_H; sleep(100)
...
readini() {
	local varname
	IniRead,settingsall,%A_ScriptName%.ini,settings,settingsall
	if settingsall {
		loop,parse,settingsall,`,
			if varname {
				global	(%varname%):= a_loopfield
				varname:= ""
